...
def fetch(query: str, params: tuple, fetch_type: str, as_dict: bool = False) -> list | dict:
    ...
    with getDatabaseConnection() as connection:
        with connection.cursor() as cursor:
            ...
            match fetch_type:
                case 'one': response: tuple | None = cursor.fetchone()
                case 'all': response: tuple = cursor.fetchall()
                case _:
